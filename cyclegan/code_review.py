...
path = '.'
dpath = os.path.sep.join([path, 'weights-cyclelossweight10-batchsize{}-imagesize{}-ep{}'.format(batch_size, image_size, how_many_epochs)])
print(dpath)
dpath_result = os.path.sep.join([dpath, 'results'])
...
netG_B.summary()
#%%
netD_B_predict_fake = netD_B(fake_B) # 7*7*1
rec_A = netG_B(fake_B)
netD_A_predict_fake = netD_A(fake_A) # 7*7*1
rec_B = netG_A(fake_A)
...
adam = Adam(lr=2e-4, beta_1=0.5, beta_2=0.999, epsilon=None, decay=0.0)
...
data_path = 'D:\\python\\dataset\\horse2zebra'
train_A = load_data(os.path.sep.join([data_path, 'trainA','*.jpg']))
train_B = load_data(os.path.sep.join([data_path, 'trainB','*.jpg']))
print(len(train_A))
print(len(train_B))

val_A = load_data(os.path.sep.join([data_path, 'testA','*.jpg']))
val_B = load_data(os.path.sep.join([data_path, 'testB','*.jpg']))
#%%
...
def display_image(X, rows=1):
    ...
    pil_X.save(os.path.sep.join([dpath,'results-'+ t+'.jpg']))
    #display(pil_X)
#%%
...
del val_batch, A, B
#%%
def poly_decay(epoch, max_epoch):
    START= 100
    START_LR = 0.0002
    END = max_epoch
    new_lr = START_LR - (START_LR * ((epoch-START)/(END-START)))
    return new_lr
...
logs = {'epoch':[],'G_loss':[],'D_A_loss':[],'D_B_loss':[], 'lr':[]}
curr_epoch = 0
while epoch_count < how_many_epochs: 
    ...
    if curr_epoch+1 == epoch_count:
        if epoch_count >= 100:
            new_lr = poly_decay(epoch_count, how_many_epochs)
            K.set_value(netG_train_function.optimizer.lr, new_lr)
            K.set_value(netD_B_train_function.optimizer.lr, new_lr)
            K.set_value(netD_A_train_function.optimizer.lr, new_lr)
            
    g_loss = netG_train_function.train_on_batch([A, B], target_label)
    
    d_b_loss = netD_B_train_function.train_on_batch([B, _fake_B], target_label)
    d_a_loss = netD_A_train_function.train_on_batch([A, _fake_A], target_label)
   
    iteration_count+=1
    
    
    if curr_epoch+1 == epoch_count:
        logs['epoch'].append(epoch_count)
        logs['G_loss'].append(g_loss)
        logs['D_A_loss'].append(d_a_loss)
        logs['D_B_loss'].append(d_b_loss)
        logs['lr'].append(K.eval(netG_train_function.optimizer.lr))
        ...
        curr_epoch = epoch_count
        if curr_epoch % 10 == 0:

            save_name = os.path.sep.join([dpath,'{}' + str(curr_epoch) + '.hdf5'])
            netG_A.save(save_name.format('tf_G_A_model'))
            netG_B.save(save_name.format('tf_G_B_model'))
...
df.to_csv(os.path.sep.join([dpath,'logs.csv']))